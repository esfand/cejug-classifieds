...
if [ $? -eq 0 ] ; then
    ...
    if [ -z "$CLEAN" ]; then jdbc/arenaPool
        asadmin create-jdbc-connection-pool \
                    ...
                    --property "User=$MYSQL_USER:Password=$MYSQL_PASSWORD:URL=$DB_URL" \
                jdbc/classifiedsPool
    fi
    echo
    echo "-------- JDBC Data Source jdbc/classifieds"
    asadmin --user=$ASADMIN_USER delete-jdbc-resource jdbc/classifieds
    if [ -z "$CLEAN" ]; then
        asadmin --user=$ASADMIN_USER \
	        create-jdbc-resource \
		        --connectionpoolid jdbc/classifiedsPool \
			                   jdbc/classifieds
    fi
    ...
    if [ -z "$CLEAN" ]; then
        asadmin create-jms-resource \
                    ...
                    --description="Cejug-Classifieds Registration Connection Factory." \
            RegistrationQueueConnectionFactory
    fi
    echo
    ...
    if [ -z "$CLEAN" ]; then
        asadmin create-jms-resource \
                    ...
                    --description="Queue used to register Cejug-Classifieds users." \
                RegistrationQueue
    fi
    ...
    if [ -z "$CLEAN" ]; then
        asadmin create-jms-resource \
                        --restype=javax.jms.QueueConnectionFactory \
                        --description="Cejug-Classifieds Notification Connection Factory." \
                    NotificationQueueConnectionFactory
    fi
    echo
    ...
    if [ -z "$CLEAN" ]; then
        asadmin create-jms-resource \
                        --restype=javax.jms.Queue \ 
                        --description="Queue used to notify Cejug-Classifieds users about their registration." \
                    NotificationQueue
    fi
    echo
    ...
    if [ -z "$CLEAN" ]; then
        asadmin --interactive=false \
                create-javamail-resource \
                        --mailhost=$MAIL_HOST \
                        --mailuser=$MAIL_USER \
                        --fromaddress=$MAIL_FROM  \ 
                        --enabled=true \
                        --description="e-Mail account used to confirm the registration of the Arena PUJ users" \
                        --storeprotocol=imap \
                        --storeprotocolclass=com.sun.mail.imap.IMAPStore \
                        --transprotocol smtp \
                        --transprotocolclass com.sun.mail.smtp.SMTPSSLTransport \
                        --property mail-smtp.user=$MAIL_SMTP_USER:\
                                   mail-smtp.port=465:\
                                   mail-smtp.password=$MAIL_SMTP_PASSWORD:\
                                   mail-smtp.auth=true:\
                                   mail-smtp.socketFactory.fallback=false:\
                                   mail-smtp.socketFactory.class=javax.net.ssl.SSLSocketFactory:\
                                   mail-smtp.socketFactory.port=$MAIL_SMTP_PORT:\
